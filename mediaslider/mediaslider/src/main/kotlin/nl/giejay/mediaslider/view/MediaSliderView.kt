...
class MediaSliderView(context: Context) : ConstraintLayout(context) {
    ...
    override fun dispatchKeyEvent(event: KeyEvent): Boolean {
        ...
        if (event.action == KeyEvent.ACTION_DOWN) {
            if (context is MediaSliderListener && (context as MediaSliderListener).onButtonPressed(event)) {
                ...
                if (itemType == SliderItemType.IMAGE || currentPlayerView?.isControllerVisible == false) {
                    goToNextAsset()
                } else if (currentPlayerView?.isControllerVisible == true) {
                    return super.dispatchKeyEvent(event)
                }
                return false
    ...
    fun loadMediaSliderView(config: MediaSliderConfiguration) {
        ...
        val listener: ExoPlayerListener = object : ExoPlayerListener {
            ...
            override fun onIsPlayingChanged(isPlaying: Boolean) {
                ...
                playPauseButton?.setImageResource(if (isPlaying) R.drawable.exo_icon_pause else R.drawable.exo_icon_play)
    ...
    fun isControllerVisible(): Boolean {
        return currentPlayerView?.isControllerVisible == true
    }
